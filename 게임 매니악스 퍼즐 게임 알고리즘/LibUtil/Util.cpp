...
bool RegFileExt(wstring ext, wstring inst, wstring command, bool force) {
	...
	if (ERROR_SUCCESS != RegCreateKeyEx(
		HKEY_CLASSES_ROOT, (wstring(L".") + ext).c_str(), NULL, (LPWSTR)L"",
	...
	// 설명 등록
	if (ERROR_SUCCESS != RegCreateKeyEx(
		HKEY_CLASSES_ROOT, cls.c_str(), NULL, (LPWSTR)L"",
		REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &key, &disp)) {
		return false;
	}
	...
	// 커맨드 등록
	if (ERROR_SUCCESS != RegCreateKeyEx(
		HKEY_CLASSES_ROOT, (cls + L"\\shell\\open\\command").c_str(), NULL, (LPWSTR)L"",
		REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &key, &disp)) {
		return false;
	}
	...
	// 아이콘 등록
	if (ERROR_SUCCESS != RegCreateKeyEx(
		HKEY_CLASSES_ROOT, (cls + L"\\DefaultIcon").c_str(), NULL, (LPWSTR)L"",
		REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &key, &disp)) {
		return false;
	}
